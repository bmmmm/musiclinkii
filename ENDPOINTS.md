...
| `musicbrainz.org/ws/2/release/?query=barcode:{upc}` + `release/{id}?inc=url-rels` | UPC (from Deezer album) → release URL relations → **the only keyless path to an exact Spotify ALBUM link**; Discovery also yielded exact Tidal, Deezer and Qobuz albums | none | `*` | 2026-08-20 | `js/adapters.mjs` `findLinksByUpc`, `js/enrich.mjs` `enrichByCode('upc')` | The search response carries **no url-rels inline** — the two-step (search, then per-release lookup) is mandatory. Capped at 2 release lookups ≈ 3 throttled MB calls through the shared `mbJson` queue. MB stores Apple links id-prefixed (`itunes.apple.com/{cc}/album/id{n}`) — `parseApple` accepts that form |
