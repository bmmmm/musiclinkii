...
| `musicbrainz.org/ws/2/url/?resource=` + `/recording/{id}` or `/release/{id}` | Tidal fallback: URL relation → recording (tracks) / release (albums, verified: album 1550545 → "Discovery") → artist credits | none (UA not settable from browsers) | `*` | 2026-08-20 | `js/adapters.mjs` `fromTidal` | [MusicBrainz API](https://musicbrainz.org/doc/MusicBrainz_API), rate ~1 req/s, 503 on excess |
