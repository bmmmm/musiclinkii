...
Visual comparison is a separate, explicit click. With two to five OCR candidates
it compares their artwork; without a catalog match it can search the shipped
long-tail pilot. The selected image is passed to the model as a local `blob:`
URL. Neither the image nor its vector is used as a request body or remote URL,
and the person scanning still confirms a result.
...
| `huggingface.co/Xenova/dinov2-{small,base,large}/resolve/main/…` | selectable q4 DINOv2 configuration, processor and ONNX weights: Small 15,035,808 bytes, Base 56,429,520 bytes, Large 194,059,408 bytes; each selected variant has a separate app-owned Cache Storage entry until its delete button or browser site-data controls remove it. The static 12-cover pilot index remains Small-only. | ordinary model asset requests, **never the selected image** | 2026-09-04 (official repository file APIs; Small verified in real Chrome) | `js/visual-match.mjs` `loadExtractor` | [Small](https://huggingface.co/Xenova/dinov2-small), [Base](https://huggingface.co/Xenova/dinov2-base), [Large](https://huggingface.co/Xenova/dinov2-large), [base-model license](https://huggingface.co/facebook/dinov2-small) |
...
| same-origin `assets/vinyl-index/{manifest,shard-*}` | versioned Int8 DINOv2 vectors and matching release metadata for the local long-tail pilot | ordinary static GET requests; **no query vector and no selected image** | 2026-09-04 (12-cover Chrome retrieval) | `js/vinyl-index.mjs` `searchVinylCatalog` | shard and metadata counts must agree; rebuild with `scripts/build-vinyl-index-assets.mjs` |
| `coverartarchive.org/release/{mbid}/front-500` | result thumbnails after local pilot retrieval | browser image GET with `no-referrer`; **never the selected image** | 2026-09-04 (pilot result list) | `js/app.mjs` `renderScanCandidates` | metadata comes from the static pilot shard; a failed thumbnail does not affect selection |
